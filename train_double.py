...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights1, weights2, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights1, opt.weights2, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
    ...
    data_dict = None
    if RANK in {-1, 0}:
        loggers = Loggers(save_dir, weights1, opt, hyp, LOGGER)  # loggers instance
    ...
    # Model
    check_suffix(weights1, '.pt')  # check weights
    pretrained = weights1.endswith('.pt')
    def load_model(w):
        ckpt = torch.load(w, map_location='cpu')  # load checkpoint to CPU to avoid CUDA memory leak
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        exclude = ['anchor'] if (cfg or hyp.get('anchors')) and not resume else []  # exclude keys
        csd = ckpt['model'].float().state_dict()  # checkpoint state_dict as FP32
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f'Transferred {len(csd)}/{len(model.state_dict())} items from {w}')  # report
        return model

    model1 = load_model(weights1)
    model2 = load_model(weights2)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights1', type=str, default=ROOT / 'yolov5s.pt', help='Context1 model')
    parser.add_argument('--weights2', type=str, default=ROOT / 'yolov5s.pt', help='Context2 model')
...
def main(opt, callbacks=Callbacks()):
    ...
    opt.data, opt.cfg, opt.hyp, opt.weights1, opt.weights2, opt.project = \
        check_file(opt.data), check_yaml(opt.cfg), check_yaml(opt.hyp),\
            str(opt.weights1), str(opt.weights2), str(opt.project)  # checks
    assert len(opt.weights1) and len(opt.weights2), 'either --cfg or --weights must be specified'
